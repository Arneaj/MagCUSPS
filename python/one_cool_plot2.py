...
MP = cusps.get_interest_points(
    ...
    Rho_processed,
    theta_min=0.0, theta_max=np.pi*0.9,  
    nb_theta=40, nb_phi=100,
    dx=0.1, dr=0.1,
    alpha_0_min=0.5, alpha_0_max=0.7, nb_alpha_0=4,
...
ax.set_xlabel(r"$x \in [-30; 128] R_E$", fontsize=12, labelpad=6.0)
ax.set_ylabel(r"$z \in [-58; 58] R_E$", fontsize=12, labelpad=6.0)


fig.set_figwidth(7)
fig.set_figheight(5)

fig.suptitle(r"$||\mathbf{J}|| \in (\mathbf{P}_{\text{Earth}},\hat x, \hat y)$", fontsize=15)
...
ax.scatter( X_MP_plot, Z_MP_plot, s=1.5, c=MP[is_in_plane_MP,3], cmap="Spectral", zorder=1, label="Magnetopause" )
# ax.scatter( X_BS_plot, Z_BS_plot, s=1.5, color="green", zorder=1, label="Bow-shock" )
# ax.set_title( f"Magnetopause and Bow-shock interest points results.\nColours of MP interest points range from blue ($1$) to red ($0$)" )
...
plt.savefig("../images/one_cool_plot.svg")
